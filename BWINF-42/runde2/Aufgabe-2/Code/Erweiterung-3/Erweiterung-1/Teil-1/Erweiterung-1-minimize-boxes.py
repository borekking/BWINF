...
def ilp(A, cliques, types, styles, best_amount, left, right, message=0, time=None):
    # --- Init ILP Solver ---
    # 1) Create Variables
    ...
    # The object is now given by the amount of empty boxes:s
    PROBLEM = LpProblem("Problem-1", LpMaximize)
    objective = lpSum(empty)
    PROBLEM += objective
    ...
    # 3.3) Empty boxes
    D3 = 1000000 # Make sure D3 > Sum of all element of A
    for index in range(cnt):
        sum = 0
        for i in range(types):
            for j in range(styles): 
    ...
    # 3.4) New constaint for making sure the amount of boxes wanted will be achieved
    all_sum = 0
    for index in range(cnt):
        for i in range(types):
            for j in range(styles): 
...
def solve(types, styles, best_amount, left_bound, right_bound, A, K):
    ...
    PROBLEM, boxes, empty, k = ilp(A, cliques, types, styles, best_amount, left_bound, right_bound, msg, timeLimit)
...
if __name__ == '__main__':
    ...
    best_amount = int(input("Please enter the biggest amount of used Clothes: "))

    types, styles, A, K  = read_problem(path)
    solve(types, styles, best_amount, left_bound, right_bound, A, K)
